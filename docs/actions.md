...
While the command runs, the app says so. When it is done, it shows the last line the command printed; when it fails (a non-zero exit), the last line of its error output. A link on that line, to a web page or an `obsidian://` note, gets an **Open** button.
...
- Print one line when done: it is shown to the user. Put a link in that line to get an Open button. Exit non-zero on failure and write the reason to stderr; its last line is shown.
